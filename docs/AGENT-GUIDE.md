...
7. **Prepare a recipe for groceries:** Use a `recipe` list. Supply exact `recipeIngredient` strings from Schema.org JSON-LD when present; otherwise save the URL and add verified ingredients later. Call `search_my_recipes` and `get_recipe_grocery_handoff` for a permissioned ingredient view. Do not proceed when `ready_for_grocery_order` is false. Review quantities, pantry items, and substitutions with the user before passing ingredients to an external Instacart or other grocery agent. ShopsApp does not place the order.

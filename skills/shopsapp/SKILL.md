...
2. When the user shares a URL, call `capture_url` with the exact URL, an accurate title, optional variant and quantity, a category if known, and a stable idempotency key if retrying. Use `other` when the category is unclear. Include a GTIN/UPC/EAN only if it is actually supplied by trustworthy product data; never invent one. A valid check digit is not proof that the seller's product claim is correct. Preserve the original URL, including any existing affiliate or tracking code. Do not follow it and substitute a canonical URL.
   Inspect the returned `link_assessment`. `inspiration` means the URL looks like a social post or video, not a direct product page; help identify the item and find a seller before proposing a purchase. `unverified` means the URL alone was inconclusive; check the destination with your own browsing tools before calling it shoppable. `likely_product` is only a URL or structured-data hint, never a live price, stock, or checkout guarantee. `checked_live` is always false in ShopsApp. If you actually observed Schema.org Product data on the page, pass `page_type="product"`; never assert this based on a title alone. The `get_handoff` response gives a `next_step` for these cases. A non-shoppable link may still be useful inspiration and may be saved as such.
